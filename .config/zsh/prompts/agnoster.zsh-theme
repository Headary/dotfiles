...
prompt_user() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CONNECTION" ]]; then
    prompt_segment blue black " %(!.%{%F{yellow}%}.)$user "
  fi
}
...
prompt_git() {
  ...
  if ( which git > /dev/null 2>&1 ); then
      ...
      prompt_segment black white " $branch $symbol "
			#echo "$branch $symbol"	
		fi
	fi
}
